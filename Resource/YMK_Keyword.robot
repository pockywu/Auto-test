*** Keywords ***
Open App
    Open Application    ${REMOTE_URL}    platformName=${platformName}    platformVersion=${platformVersion}    deviceName=Android    automationName=${automationName}    appPackage=${appPackage}
...
Switch Country
    ...
    \    Swipe    400    1000    400    300    400
    \    ${count}    Get Matching Xpath Count    xpath=//*[contains(@text, 'Country/Region')]
    \    Exit For Loop If    ${count}>0
    Click text    Country/Region
    Wait Until Page Contains Element    com.cyberlink.youcammakeup:id/country_picker_search_bar
    Input text    com.cyberlink.youcammakeup:id/country_picker_search_bar    ${country}
    Press Keycode    66
    Click Element    com.cyberlink.youcammakeup:id/country_name
    Click Element    com.cyberlink.youcammakeup:id/alertDialog_buttonPositive

Syetem language
    [Arguments]    ${language}
    ${photomakeup}=    Set Variable If    "${language}"=="CHT"    照片上妝
    Set Global Variable    ${photomakeup}
    ${about}=    Set Variable If    "${language}"=="ENU"    About    "${language}"=="CHT"    關於
